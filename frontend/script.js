const loadEvent = async () => {
   ...
   const head = () => {
    ...
    const bookHTML = books.cards.map(x => `
    <div class="books">
        <p class="number">${x.number}</p>
        <h4 class="sub">${x.sub}</h4>
        <h4 class="title">${x.title}</h4>
        <p class="text">${x.text}</p>
        <button class="button">read more<span class="material-symbols-outlined">arrow_forward</span></button>
    ...
    rootElement.insertAdjacentHTML("beforeend", bookHTML.join(''))
